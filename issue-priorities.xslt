...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="existingIssues"
		select="$compactedIssues/issue[@githubIssue and @status='open']" />

	<xsl:template match="/">
		<xsl:result-document href="{$missingissuefile}">
			<xsl:copy-of
				select="$compactedIssues/issue[not(@githubIssue)]" />
		</xsl:result-document>
		<xsl:variable name="prios"
			select="zenta:prioritizeIssues(/dummy,$existingIssues,1)" />
	...
	<xsl:template
		match="@*|*|processing-instruction()|comment()" mode="justname">
		<xsl:copy>
			<xsl:apply-templates
		...
		<xsl:choose>
			<xsl:when test="$depth > 5 or not($nonprioritized)">
				<result>
					...
					<xsl:apply-templates select="$prioritized"
						mode="justname" />
					<left>
						<xsl:apply-templates select="$nonprioritized"
							mode="justname" />
					</left>
				</result>
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="newprioritized"
					select="$nonprioritized[not(value/@target = $nonprioritized/@operationid)]" />
				<xsl:variable name="newnonprioritized"
					select="$nonprioritized[not(@errorID = $newprioritized/@errorID)]" />
				<xsl:copy-of
					select="zenta:prioritizeIssues($prioritized|$newprioritized,$newnonprioritized,$depth+1)" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
	<xsl:template match="entry">
		<xsl:if test="object/testcase">
			...
			<xsl:variable name="issue"
				select="$issues//link[@url=$errorUrl]/.." />
			<xsl:variable name="operationid"
				select="object/testcase/@operationid" />
			<issue>
				<xsl:copy-of select="object/testcase/@*" />
				<xsl:copy-of select="@*" />
				<xsl:if test="$issue/@url">
					<xsl:attribute name="githubIssue"
						select="$issue/@url" />
					<xsl:attribute name="issueTitle"
