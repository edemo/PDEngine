...
RUN apt-get update
RUN apt-get -y install openjdk-8-jdk wget git xvfb unzip docbook-xsl make firefox vnc4server dblatex libwebkitgtk-3.0-0 libswt-webkit-gtk-3-jni python-yaml python-pip python-dateutil zip debhelper devscripts zenta zenta-tools maven haveged
...
#run haveged instead in a production system!
RUN rm /dev/random; cp -a /dev/urandom /dev/random

RUN git clone https://github.com/edemo/PDEngine.git
RUN cp /usr/local/lib/*.jar PDEngine/tools
RUN Xvnc4 -SecurityTypes none :0 & export DISPLAY=:0;cd PDEngine; make
