...
public interface CanModifyVote {
  ...
  default Vote getModifiableVote(final VoteAdminInfo voteAdminInfo, final VoteService voteService) {
  ...
  default void checkIfVoteIsAddinable(final Vote vote, final RuntimeException exception) {
    if (!vote.getParameters().isAddinable())
      throw exception;
  }

  default void checkIfUserIsTheSame(final Choice votesChoice, final String userName,
