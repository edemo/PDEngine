...
@Service
public class BeatTableTransitiveClosureServiceImpl implements BeatTableTransitiveClosureService {
  ...
  public BeatTable computeTransitiveClosure(final BeatTable beatTable) {
    ...
    keyCollection.forEach(i -> keyCollection.forEach(j -> computeMinimalRoute(result, i, j)));
    return result;
  }

  private void computeMinimalRoute(final BeatTable beatTable, final String choice1,
      final String choice2) {
    if (!choice1.equals(choice2))
      beatTable.getKeyCollection().forEach(k -> selectShorterPath(beatTable, choice1, choice2, k));
  }

  private void selectShorterPath(final BeatTable beatTable, final String choice1,
      final String choice2, final String middleChoice) {
    if (!Set.of(choice1, choice2).contains(middleChoice))
      beatTable.setElement(choice1, choice2,
