...
public interface ContainingBeats extends Matrix<String, Pair> {
  ...
  default void initialize(final List<CastVote> castVotes) {
    ...
    for (CastVote castVote : castVotes) {
      ...
      for (
          int columnIndex = 0; columnIndex < preferences.size(); columnIndex++
      ) {
        RankedChoice column = preferences.get(columnIndex);
        for (
            int rowIndex = columnIndex + 1; rowIndex < preferences.size();
            rowIndex++
        ) {
  ...
  default void increasePairValue(
      final List<RankedChoice> preferences, final int column, final int row
  ) {
    Pair value1 = getPair(
        preferences.get(column).choiceId, preferences.get(row).choiceId
    );
    setElement(
        preferences.get(column).choiceId, preferences.get(row).choiceId,
    ...
    Pair value2 = getPair(
        preferences.get(row).choiceId, preferences.get(column).choiceId
    );
    setElement(
        preferences.get(row).choiceId, preferences.get(column).choiceId,
