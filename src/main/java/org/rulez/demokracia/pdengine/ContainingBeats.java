			...
			for (int i = 0; i < preferences.size(); i++) {
				for (int j = i + 1; j < preferences.size(); j++) {
					if(preferences.get(i).rank > preferences.get(j).rank) {
						...
						setElement(preferences.get(i).choiceId, preferences.get(j).choiceId, new Pair(value1.winning + 1, value1.losing));
						
						Pair value2 = getPair(preferences.get(j).choiceId, preferences.get(i).choiceId);
						setElement(preferences.get(j).choiceId, preferences.get(i).choiceId, new Pair(value2.winning, value2.losing + 1));
						...
						setElement(preferences.get(j).choiceId, preferences.get(i).choiceId, new Pair(value1.winning + 1, value1.losing));
