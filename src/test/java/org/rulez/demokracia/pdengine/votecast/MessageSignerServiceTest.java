...
public class MessageSignerServiceTest {
  ...
  public void setUp() throws Exception {
    ...
    when(keyProvider.getPrivateKey("alias", "pass"))
        .thenReturn(keys.getPrivate());
  }

  @Test
  public void test_signature_is_valid() throws Exception {
    String signatureOfMessage =
        messageSignerService.signatureOfMessage(SERIAL_PLAN);
    Signature signature = Signature.getInstance("SHA256WithRSA");
    signature.initVerify(keys.getPublic());
    signature.update(SERIAL_PLAN.getBytes());
    assertTrue(
        signature.verify(Base64.getDecoder().decode(signatureOfMessage))
    );
  }
