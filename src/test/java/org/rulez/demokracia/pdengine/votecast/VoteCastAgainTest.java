...
public class VoteCastAgainTest extends CastVoteTestBase {
  ...
  private void assertChangeVoteOfUser(final String userName, final boolean hasVote) {
    when(authService.getAuthenticatedUserName()).thenReturn(userName);

    int expectedVoteNumber = vote.getVotesCast().size() + (hasVote ? 0 : 1);

    castVoteService.castVote(vote.getId(), ballot, List.of(new RankedChoice(CHOICE_B.getId(), 1)));
  ...
  private String getFirstChoiceIdOfUser(final String user) {
    return vote.getVotesCast().stream().filter(c -> user.equals(c.getProxyId())).findFirst().get()
        .getPreferences().get(0).getChoiceId();
  }
}
