...
@TestedBehaviour(
  "The vote receipt contains the ballot cast and the cast vote identifier"
)
...
public class VoteCastReceiptTest extends CastVoteTestBase {
  ...
  public void cast_vote_returns_the_cast_vote_id() {
    CastVote receipt = castVoteService.castVote(
        vote.getId(), ballot,
        List.of(new RankedChoice(CHOICE_B.getId(), 1))
    );
    assertEquals(
        vote.getVotesCast().get(0).getSecretId(), receipt.getSecretId()
    );
  }
  ...
  public void cast_vote_returns_the_cast_vote_preferences() {
    ...
    assertEquals(
        vote.getVotesCast().get(0).getPreferences().get(0).getChoiceId(),
