...
public class VoteShowTest extends CreatedDefaultVoteRegistry {
  ...
  public void setUp() {
    ...
    voteManager.addChoice(
        new VoteAdminInfo(adminInfo.voteId, adminInfo.adminKey), CHOICE_NAME, USER
    );
  ...
  public void the_name_attribute_contains_the_name_of_the_vote() {
    JsonObject result =
        voteManager.showVote(new VoteAdminInfo(voteId, adminKey));
  ...
  public void the_choices_attribute_contains_the_choices_of_the_vote() {
    ...
    assertEquals(
        vote.choices.size(), result.get(CHOICES).getAsJsonObject().entrySet().size()
    );
  }
  ...
  private void assertChoicesJsonContainsAllUserNames(
    ...
    choicesJson.entrySet()
        .forEach(
            e -> assertJsonContainsUserName(e.getValue(), choices.get(e.getKey()).userName)
        );
  }
