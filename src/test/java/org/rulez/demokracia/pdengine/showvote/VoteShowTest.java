...
public class VoteShowTest extends ShowVoteJSONTestBase {
  ...
  public void the_choices_attribute_contains_the_choices_of_the_vote() {
    assertEquals(
        vote.getChoices().size(), result.get(CHOICES).getAsJsonObject().entrySet().size()
    );
  }
  ...
  public void
    ...
    assertJsonContainsAllElement(
        jsonCountedAssurances, vote.getCountedAssurances()
    );
  }
  ...
  public void
    ...
    assertJsonContainsAllElement(
        jsonNeededAssurances, vote.getNeededAssurances()
    );
  }
