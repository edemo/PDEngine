...
public class CalculateWinnersTest {
  ...
  public void calculate_winners_returns_none_of_the_ignored_choices() {
    final Collection<String> ignoredChoices = List.of("A");
    final List<String> winners =
        winnerCalculator.calculateWinners(beatPathTable, ignoredChoices);
  ...
  private void assertNonbeatensAreWinner(final List<String> winners) {
    assertTrue(
        beatPathTable.getKeyCollection().stream()
  ...
  private boolean
    ...
    for (final String player2 : beatPathTable.getKeyCollection()) {
      final Pair forward = beatPathTable.getElement(player1, player2);
      final Pair backward = beatPathTable.getElement(player2, player1);
      if (forward.compareTo(backward) < 0)
        return false;
    }
    return true;
  }
