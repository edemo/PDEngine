...
public class ChoiceModificationValidatorServiceTest extends ThrowableTester {

  private static final String CHOICE_ADDED_BY_DIFFERENT_USER_MESSAGE =
      "Choice modification disallowed: adminKey is user, and the choice was added by a different user";
  ...
  public void userAdmin_cannot_modify_choice_if_canAddin_is_false() {
    assertThrows(
        () -> underTest
            .validateModification(new VoteAdminInfo(vote.getId(), USER),
                vote, choice
            )
  ...
  public void proper_voteId_choiceId_and_adminKey_does_modify_choice() {
    underTest.validateModification(
  ...
  public void
    ...
    assertThrows(
        () -> underTest
            .validateModification(new VoteAdminInfo(vote.getId(), USER), vote,
                choice
            )
    )
        .assertMessageIs(
  ...
  public void
    ...
    underTest
        .validateModification(new VoteAdminInfo(vote.getId(), USER), vote, choice);
  }
}
