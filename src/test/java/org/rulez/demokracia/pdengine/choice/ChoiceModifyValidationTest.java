...
public class ChoiceModifyValidationTest extends ChoiceTestBase {
	...
	private Choice choice = new Choice("choiceName", USER);
	...
	public void invalid_voteId_is_rejected() {
		...
		assertThrows(
				() -> choiceService.modifyChoice(new VoteAdminInfo(invalidvoteId, vote.getAdminKey()), choice.getId(),
						NEW_CHOICE_NAME)
	...
	public void invalid_adminKey_is_rejected() {
		...
		assertThrows(
				() -> choiceService.modifyChoice(new VoteAdminInfo(vote.getId(),
						invalidAdminKey), choice.getId(), NEW_CHOICE_NAME)).assertMessageIs("Illegal adminKey");
	}
	...
	public void when_ballots_are_already_issued_choices_cannot_be_modified()
	{
		...
		assertThrows(() -> choiceService.modifyChoice(new VoteAdminInfo(vote.getId(),
				vote.getAdminKey()), choice.getId(), "something else"))
	...
	public void userAdmin_cannot_modify_choice_if_canAddin_is_false() {
		...
		assertThrows(() -> choiceService.modifyChoice(new VoteAdminInfo(vote.getId(),
				USER), choice.getId(), NEW_CHOICE_NAME))
