...
public class CastVoteMockTestHelper {
  ...
  private static List<CastVote> castVoteMocksWithIncrementalAssurances(final int limit) {
    return IntStream.range(1, limit).boxed().map(CastVoteMockTestHelper::nFirstIntAsString)
        .map(CastVoteMockTestHelper::castVoteMockFromAssurances).collect(Collectors.toList());
  }

  private static List<String> nFirstIntAsString(final Integer limit) {
    return IntStream.range(0, limit).boxed().map(Object::toString).collect(Collectors.toList());
  }
