...
public class CreatedDefaultChoice extends CreatedDefaultVoteRegistry {
  ...
  protected String addMyChoice() {
    return voteManager.addChoice(
        new VoteAdminInfo(adminInfo.voteId, adminInfo.adminKey), "choice2", null
    );
  }
  ...
  protected void assertValidationFailsWithMessage(final String message) {
    assertThrows(() -> {
      voteManager
          .endorseChoice(new VoteAdminInfo(adminInfo.voteId, adminInfo.adminKey), choiceId, "testuserke");
    })
  ...
  protected void
      addCastVote(final String proxyId, final List<RankedChoice> theCastVote) {
