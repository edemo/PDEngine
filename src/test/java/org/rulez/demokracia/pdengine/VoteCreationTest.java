...
public class VoteCreationTest extends CreatedDefaultVoteRegistry {
  ...
  public void create_creates_a_vote_with_neededAssurances() {
    assertEquals(
        neededAssurances.size(), voteManager.getVote(adminInfo.voteId).neededAssurances.size()
    );
  }
  ...
  public void neededAssurances_contains_the_assurances_of_the_input() {
    ...
    assertEquals(
        ASSURANCE_NAME, voteManager.getVote(secondVote.voteId).neededAssurances.get(0)
    );
  }
  ...
  public void create_creates_a_vote_with_countedAssurances() {
    assertEquals(
        0, voteManager.getVote(adminInfo.voteId).countedAssurances.size()
    );
  }
  ...
  public void countedAssurances_contains_the_assurances_of_the_input() {
    ...
    VoteAdminInfo secondVote = createAVote();
    assertEquals(
        ASSURANCE_NAME, voteManager.getVote(secondVote.voteId).countedAssurances.get(0)
    );
  }
  ...
  public void create_creates_a_vote_with_minEndorsements() {
    assertEquals(
        0, voteManager.getVote(adminInfo.voteId).parameters.minEndorsements
    );
  }
  ...
  public void minEndorsements_is_the_same_what_is_given_in_create() {
    ...
    VoteAdminInfo secondVote = createAVote();
    assertEquals(
        minEndorsements, voteManager.getVote(secondVote.voteId).parameters.minEndorsements
    );
  }
  ...
  public void create_creates_a_vote_with_adminKey() {
    assertEquals(
        adminInfo.adminKey, voteManager.getVote(adminInfo.voteId).adminKey
    );
  }
}
