...
public class ComputedVoteTest {
	...
	public void setUp() {
		...
		.thenReturn(BeatTableTestHelper.createTransitiveClosedBeatTable(keys));
		when(voteResultComposer.composeResult(any())).thenReturn(VoteResultTestHelper.createVoteResults());
	...
	public void compute_vote_should_create_initial_matrix_with_full_key_set() {
		assertEquals(Set.of("name1", "name2", "name3"), Set.copyOf(computedVote.getBeatTable().getKeyCollection()));
	}
	...
	public void transitive_closure_done_on_beat_path_matrix() {
		assertBeatTableEquals(createTransitiveClosedBeatTable(),
